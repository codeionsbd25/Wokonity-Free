(function () {
  ...
  const { Component, useEffect, useMemo, useRef, useState } = wp.element;
  ...
  const hasProFeature = (feature) =>
    !!WORKONITY.proActive &&
    asArray(WORKONITY.proFeatures).indexOf(feature) !== -1;
  ...
  const calculateShiftTimeline = (shift) => {
    const start = shiftTimeMinutes(shift.start_time),
      ...
      late = shift.late_after_time
        ? shiftTimeMinutes(shift.late_after_time)
        : null,
    ...
    if (shift.late_after_time && late === null)
      return {
        valid: false,
    ...
    if (
      lateAbsolute !== null &&
      (lateAbsolute < start || lateAbsolute > endAbsolute)
    )
      return {
        valid: false,
        error: "Late After Time must fall between the shift start and end.",
      };
    if (autoAbsolute < endAbsolute)
      return {
        valid: false,
        error:
          "Auto clock-out must be at or after the shift end. Choose a next-day time for an overnight shift.",
      };
    return {
      valid: true,
      endDay,
        ...
        return h(
          ...
          h(
            ...
            this.state.error.message ||
              "A dashboard error occurred. Please refresh and try again.",
          ),
          h(
            "button",
            {
  ...
  function toast(type, message) {
    ...
    region.classList.toggle(
      "workonity-toast-region-dark",
      !!document.querySelector(".workonity-app.workonity-dark"),
    );
  ...
  function isProLicenseError(error) {
    ...
    return (
      code === "workonity_professional_required" ||
      code === "workonity_pro_feature_required" ||
      message.indexOf("active workonity professional or agency license") !== -1
    );
  }

  function showProRequirement(error) {
    const feature = String(
      (error && error.data && error.data.feature) ||
        (error && error.feature) ||
        "",
    ).trim();
    window.dispatchEvent(
      new CustomEvent("workonity:pro-required", {
        detail: {
          feature,
        },
      }),
  ...
  function notifyError(error, fallback) {
    if (isProLicenseError(error)) {
      console.info(
        "WORKONITY Professional feature requested without an active entitlement.",
        error,
      );
    ...
    const message =
      error && error.message
        ? error.message
        : fallback || (typeof error === "string" ? error : "Request failed.");
  ...
  function mutationMessage(options) {
    const requestPath = String(
      (options && (options.path || options.url)) || "",
    );
    const method = String((options && options.method) || "GET").toUpperCase();
    if (requestPath.indexOf("/attendance/clock") !== -1)
      return "Attendance updated.";
    if (requestPath.indexOf("/attendance/corrections") !== -1)
      return "Attendance correction updated.";
    if (requestPath.indexOf("/attendance/manual") !== -1)
      return "Manual attendance saved.";
    if (requestPath.indexOf("/me/theme") !== -1)
      return "Theme preference saved.";
    if (requestPath.indexOf("/me/profile") !== -1) return "Profile updated.";
    if (requestPath.indexOf("/employees") !== -1)
      return method === "POST" ? "Employee created." : "Employee updated.";
    if (requestPath.indexOf("/departments") !== -1)
      return method === "POST" ? "Department added." : "Department updated.";
    if (requestPath.indexOf("/designations") !== -1)
      return method === "POST" ? "Designation added." : "Designation updated.";
    if (requestPath.indexOf("/shifts") !== -1)
      return method === "POST" ? "Shift added." : "Shift updated.";
    if (requestPath.indexOf("/leaves/types") !== -1)
      return method === "POST" ? "Leave type added." : "Leave type updated.";
    if (requestPath.indexOf("/holidays") !== -1)
      return method === "POST" ? "Holiday added." : "Holiday updated.";
    if (
      requestPath.indexOf("/leaves/") !== -1 &&
      requestPath.indexOf("/decision") !== -1
    )
      return "Leave decision saved.";
    if (requestPath.indexOf("/leaves") !== -1)
      return "Leave request submitted.";
    if (
      requestPath.indexOf("/roles/") !== -1 &&
      requestPath.indexOf("/permissions") !== -1
    )
      return "Permissions saved.";
    if (requestPath.indexOf("/roles") !== -1)
      return method === "POST" ? "Role created." : "Role updated.";
    if (requestPath.indexOf("/approvals/") !== -1)
      return "Approval decision saved.";
    if (requestPath.indexOf("/payroll/run") !== -1)
      return "Draft payroll or payslip generated.";
    if (
      requestPath.indexOf("/payroll/payslips/") !== -1 &&
      requestPath.indexOf("/approve") !== -1
    )
      return "Payslip approved.";
    if (requestPath.indexOf("/payroll/payslips/") !== -1)
      return "Payslip updated.";
    if (requestPath.indexOf("/documents/") !== -1 && method === "DELETE")
      return "Document deleted.";
    if (requestPath.indexOf("/announcements") !== -1)
      return method === "DELETE"
        ? "Announcement deleted."
        : method === "POST"
          ? "Announcement posted."
          : "Announcement updated.";
    if (requestPath.indexOf("/notifications/") !== -1)
      return "Notification updated.";
    if (requestPath.indexOf("/devices/") !== -1)
  ...
  apiFetch.use((options, next) => {
    ...
    return next(options).then((response) => {
      if (
        ...
        !(options && options.silentSuccess)
      ) {
        notifySuccess(mutationMessage(options));
      }
      if (method && !["GET", "HEAD", "OPTIONS"].includes(method)) {
        window.dispatchEvent(
          new CustomEvent("workonity:data-changed", {
            detail: {
  ...
  function Button(props) {
    const className = ["workonity-btn", props.className]
      .filter(Boolean)
      .join(" ");
    return h(
      "button",
  ...
  function Field({
    ...
    return h(
      ...
      h("span", null, label, required ? h("em", null, "*") : null),
      input,
      help ? h("small", null, help) : null,
    );
  }

  function Checkbox({ label, checked, onChange, help, disabled, proFeature }) {
    const inferredFeature =
      String(label || "").toLowerCase() === "allow multiple breaks"
        ? "multiple_breaks"
        : "";
    ...
    return h(
      ...
      {
        className:
          "workonity-check" + (isLocked ? " workonity-check-locked" : ""),
      },
      h("input", {
        type: "checkbox",
        checked: !!checked,
    ...
    const handleChange = (nextValue) => {
      ...
      apiFetch({
        ...
        .then((result) => {
          if (!result || !result.id)
            throw new Error(
              "The new " + singular + " was not returned by the server.",
            );
          const created = {
            id: result.id,
            value: result.id,
        ...
        .catch((error) =>
          notifyError(error, "Could not add the " + singular + "."),
        )
  ...
  function Status({ value }) {
    ...
    return h(
      ...
      {
        className:
          "workonity-status workonity-status-" +
          raw.replace(/[^a-z0-9_-]/g, ""),
      },
      raw.replace(/_/g, " "),
    );
  }

  function Card({ title, value, note, tone, locked = false }) {
    return h(
      "div",
      {
        className: [
          "workonity-card",
          tone ? "workonity-card-" + tone : "",
          locked ? "workonity-card--pro-locked" : "",
        ]
          .filter(Boolean)
          .join(" "),
      },
  ...
  function EmptyState({ title, text }) {
    return h(
      ...
      h("strong", null, title || "No records found"),
      h("span", null, text || "Data will appear here once available."),
    );
  }
  ...
  const tableValue = (row, key) =>
    row && row[key] !== undefined && row[key] !== null ? String(row[key]) : "";
  const tableLabel = (value) =>
    String(value || "")
      .replace(/_/g, " ")
      .replace(/\b\w/g, (m) => m.toUpperCase());
  const tableSearchText = (row) =>
    Object.keys(row || {})
      .map((key) => {
        const value = row[key];
  ...
  function DataTable({
    ...
    const safeRenderRow =
      typeof renderRow === "function" ? renderRow : () => null;
    ...
    const autoFilters = useMemo(
      () =>
        AUTO_FILTER_FIELDS.filter(([key]) => {
          const values = Array.from(
            new Set(
              safeRows.map((row) => tableValue(row, key)).filter(Boolean),
            ),
          );
    ...
    const allFilters = asArray(filters).concat(
      autoFilters.filter(
        (auto) => !asArray(filters).some((f) => f.key === auto.key),
      ),
    );
    ...
    const visibleRows = pagination
      ? activeRows.slice((currentPage - 1) * per, currentPage * per)
      : activeRows;
    const filterSignature =
      JSON.stringify(filterValues) + JSON.stringify(dateFilterValues);
    useEffect(() => {
      setPage(1);
    ...
    const showControls =
      ...
      searchable ||
      allFilters.length > 0 ||
      safeDateFilters.length > 0 ||
      safeRows.length > per;
    const setDateFilter = (key, part, value) =>
      setDateFilterValues((current) => ({
        ...current,
        [key]: {
          ...(current[key] || {}),
          [part]: value,
    ...
    const hasActiveTools = !!(
      ...
      Object.keys(dateFilterValues).some((key) => {
      ...
      showControls
        ? h(
            ...
            allFilters.length
              ? h(
                  ...
                  allFilters.map((filter) =>
                    h(
                      ...
                      h("span", null, filter.label || tableLabel(filter.key)),
                      h(
                        "select",
                        {
                        ...
                        h(
                          ...
                          "All " +
                            String(
                              filter.label || tableLabel(filter.key),
                            ).toLowerCase(),
                        ),
                        asArray(filter.options).map((option) =>
                          h(
                            "option",
                            {
                              key: option.value,
                            ...
                            option.label || tableLabel(option.value),
                          ),
                        ),
                      ),
                    ),
              ...
              : null,
            safeDateFilters.length
              ? h(
                  "div",
                  {
                    className: "workonity-table-date-filters",
                  },
                  safeDateFilters.map((filter) =>
                    h(
                      "fieldset",
                      {
                        key: filter.key,
                      },
                      h("legend", null, filter.label || tableLabel(filter.key)),
                      h(
                        "label",
                        null,
                        h("span", null, "From"),
                        h("input", {
                          type: "date",
                          value:
                            (dateFilterValues[filter.key] || {}).from || "",
                          onChange: (e) =>
                            setDateFilter(filter.key, "from", e.target.value),
                        }),
                      ),
                      h(
                        "label",
                        ...
                        h("input", {
                          ...
                          onChange: (e) =>
                            setDateFilter(filter.key, "to", e.target.value),
                        }),
                      ),
                    ),
                  ),
      ...
      pagination && showControls
        ? h(
            ...
            h(
              ...
              h("strong", null, `Page ${currentPage} of ${totalPages}`),
              h(
                "button",
                {
                  type: "button",
  ...
  function exportTable(rows, columns, filename, format, summary) {
    ...
    if (format === "pdf") {
      const summaryHtml = summaryRows.length
        ? '<h2>Analysis</h2><div class="summary">' +
          summaryRows
            .map(
              (item) =>
                "<div><small>" +
    ...
    const xlsSummary = summaryRows.length
      ? "<table><thead><tr><th>Analysis Metric</th><th>Value</th></tr></thead><tbody>" +
        summaryRows
          .map(
            (item) =>
              "<tr><td>" +
    ...
    const csvSummary = summaryRows.length
      ? [["Analysis Metric", "Value"]]
          .concat(summaryRows.map((item) => [item.label, item.value]))
    ...
    const content =
      format === "xls"
        ? '<html xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="utf-8"></head><body>' +
          xlsSummary +
          "<table><thead><tr>" +
          header.map((v) => "<th>" + escapeXml(v) + "</th>").join("") +
          "</tr></thead><tbody>" +
          data
            .map(
              (r) =>
                "<tr>" +
                r.map((v) => "<td>" + escapeXml(v) + "</td>").join("") +
                "</tr>",
            )
            .join("") +
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          h("strong", null, "Summary for the selected filters"),
        ),
        h("small", null, "The same analysis is included in exports."),
      ),
      h(
        "div",
        {
        ...
        items.map((item, index) =>
          h(
            ...
            item.help ? h("small", null, item.help) : null,
          ),
        ),
      ),
    );
  ...
  function reportSummaryMetrics(type, summary) {
    ...
    if (type === "payroll") {
      ...
      currencies.forEach((row) => {
        ...
        const deductions =
          number(row.unpaid_leave_deduction) +
          number(row.late_deduction) +
          number(row.other_deductions);
        metrics.push(
          {
  ...
  function Layout({ me, active, setActive, children }) {
    ...
    const itemsDef = [
      ...
      [
        "settings",
        "Settings",
        ["settings.manage", "settings.branding", "settings.verification"],
      ],
      ["audit", "Audit Logs", ["audit.view"], "audit_logs"],
      [
        "imports",
        "Imports",
        ["employees.manage", "attendance.manage"],
        "imports",
      ],
      [
        "verification",
        "Verification",
        ["settings.verification", "attendance.manage"],
        "attendance_verification",
      ],
    ];
    const items = itemsDef.filter((i) => hasAny(perms, i[2]));
    const company =
      me.settings?.company_name || WORKONITY.brandName || "WORKONITY";
    ...
    const lightTokens = {
      ...
      "--workonity-sidebar-bg":
        colors.sidebar_background || me.settings?.secondary_color || "#071A3D",
    ...
    return h(
      ...
      {
        className:
          "workonity-app" + (theme === "dark" ? " workonity-dark" : ""),
          ...
          items.map((i) => {
            ...
            return h(
              "button",
              {
                ...
                disabled: locked,
                title: locked
                  ? "Available with WORKONITY Professional or Agency"
                  : "",
                onClick: () => {
                ...
                className:
                  (active === i[0] ? "active " : "") +
                  (locked ? "workonity-nav-locked" : ""),
              },
              h("span", null, i[1]),
              locked ? h("small", null, "PRO") : null,
            );
          }),
        ),
      ),
      h(
        ...
        h(
          ...
          h(
            "div",
            null,
            h("h1", null, dashboardName),
            h(
              "p",
              null,
              "HR, attendance, leaves, hierarchy, payroll, approvals, documents, and reports.",
            ),
          ),
          h(
            "div",
            ...
            h(
              ...
              h("span", null, "Logged in as"),
              h("strong", null, me.user?.name || "User"),
            ),
            WORKONITY.logoutUrl
              ? h(
                  "a",
  ...
  function Overview({ me }) {
    const s = me.summary || {};
    const perms = me.permissions || [];
    const canViewEmployeeCount = hasAny(perms, [
      "employees.view",
      "employees.manage",
    ]);
    const canViewAnnouncements =
      hasProFeature("announcements") &&
      hasAny(perms, ["announcements.view", "announcements.manage"]);
    ...
    const profileEditingEnabled =
      !!me.settings?.employee_profile_editing && !!profileEmployee;
    useEffect(() => {
      if (canViewAnnouncements)
        apiFetch({
          ...
          .catch(() => setOverviewAnnouncements([]));
    }, []);
      ...
      apiFetch({
        ...
        .then((result) => {
          ...
          return fetch(WORKONITY.root + "/me/profile/photo", {
            ...
            .then((photo) => {
              if (!response.ok || photo.code)
                throw new Error(
                  photo.message || "The profile image could not be uploaded.",
                );
              return {
                ...updated,
    ...
    return h(
      ...
      h(
        ...
        h(Card, {
          ...
          note: hasProFeature("leave_requests")
            ? "Waiting approval"
      ...
      h(
        ...
        h(
          ...
          h(
            ...
            (s.attendance_trend || []).map((d) => {
              const max = Math.max(
                1,
                Number(d.present_count) + Number(d.absent_count),
              );
              return h(
                "div",
                {
        ...
        h(
          ...
          h(
            ...
            (s.department_today || []).map((d) =>
              h(
                "div",
                {
                  key: d.department,
                },
                h("span", null, d.department),
                h(
                  "div",
                  {
                  ...
                  h("i", {
                    style: {
                      width:
                        (Number(d.present_count) /
                          Math.max(1, Number(d.total))) *
                          100 +
                        "%",
                    },
                  }),
      ...
      h(
        ...
        editingProfile
          ? h(
              ...
              h("h3", null, "Edit Basic Details and Profile Photo"),
              h(
                "div",
                {
                ...
                h(
                  ...
                  h("span", null, "Profile Photo"),
                  profileEmployee.profile_image_url
                    ? h("img", {
                  ...
                  h(
                    ...
                    null,
                    profilePhoto
                      ? `New image selected: ${profilePhoto.name}`
                      : "Choose a new image to replace the current photo. JPG, PNG, or WebP; up to 5 MB.",
                  ),
                ),
              ),
              h(
        ...
        profileEmployee
          ? h(
              ...
              h(
                ...
                null,
                h("strong", null, "Name"),
                h("span", null, fullName(profileEmployee)),
              ),
              h(
                "div",
                null,
                h("strong", null, "Email"),
                h("span", null, profileEmployee.email),
              ),
              h(
                "div",
                null,
                h("strong", null, "Employee ID"),
                h("span", null, fmt(profileEmployee.employee_code)),
              ),
              h(
                "div",
                null,
                h("strong", null, "Phone"),
                h("span", null, fmt(profileEmployee.phone)),
              ),
              h(
                "div",
                null,
                h("strong", null, "Employment Type"),
                h(
                  "span",
                  null,
                  fmt(profileEmployee.employment_type).replace(/_/g, " "),
                ),
              ),
              h(
                "div",
                null,
                h("strong", null, "Joining Date"),
                h("span", null, fmt(profileEmployee.joining_date)),
              ),
              h(
                "div",
                null,
                h("strong", null, "Department"),
                h("span", null, fmt(profileEmployee.department_name)),
              ),
              h(
                "div",
                null,
                h("strong", null, "Designation"),
                h("span", null, fmt(profileEmployee.designation_name)),
              ),
              h(
                "div",
                null,
                h("strong", null, "Role"),
                h("span", null, fmt(profileEmployee.role_name)),
              ),
              h(
                "div",
                null,
                h("strong", null, "Reporting Manager"),
                h(
                  "span",
                  null,
                  profileEmployee.reporting_manager_name ||
                    "Top level / no manager assigned",
                ),
              ),
              h(
                "div",
                null,
                h("strong", null, "Shift"),
                h("span", null, fmt(profileEmployee.shift_name)),
              ),
              h(
                "div",
                null,
                h("strong", null, "Address"),
                h("span", null, fmt(profileEmployee.address)),
              ),
              h(
                "div",
                null,
                h("strong", null, "Emergency Contact"),
                h("span", null, fmt(profileEmployee.emergency_contact)),
              ),
              h(
                "div",
                null,
                h("strong", null, "CNIC / Passport"),
                h("span", null, fmt(profileEmployee.national_id)),
              ),
              h(
                "div",
                null,
              ...
              h(
                "div",
                null,
                h("strong", null, "Salary"),
                h(
                  "span",
                  null,
                  profileEmployee.pay_basis === "hourly"
      ...
      h(
        ...
        h(
          ...
          h(PanelTitle, {
            ...
            text: hasAny(perms, ["employees.manage", "attendance.view_all"])
              ? "HR/Admin view enabled."
              : hasAny(perms, ["attendance.view_team", "leaves.approve"])
                ? "Team Lead / Manager view enabled."
                : "Employee self-service view enabled.",
          }),
          h(
            "p",
            null,
            hasAny(perms, ["payroll.manage"])
                    ...
                    overviewAnnouncements.map((item) =>
                      h(
                        ...
                        h(
                          ...
                          h("time", null, fmt(item.published_at)),
                        ),
                        h(
                          "p",
                          null,
                          (plainText(item.content).slice(0, 180) ||
                            (item.audience === "all"
                              ? "Company announcement"
                              : "Audience: " + item.audience)) +
                            (plainText(item.content).length > 180 ? "…" : ""),
                        ),
                      ),
                    ),
                  )
  ...
  function Attendance({ me }) {
    ...
    const perms = me.permissions || [];
    const load = () => {
      if (hasAny(perms, ["attendance.clock", "attendance.manage"]))
        apiFetch({
      ...
      apiFetch({
        ...
        .then(setListState(setRecords))
        .catch(console.error);
      if (hasAny(perms, ["attendance.correct", "attendance.manage"]))
        apiFetch({
          path: path("/attendance/corrections"),
        })
          .then(setListState(setCorrections))
          .catch(console.error);
      if (hasAny(perms, ["attendance.manage", "attendance.manual"]))
        apiFetch({
          path: path("/employees"),
        })
          .then(setListState(setEmployees))
          .catch(console.error);
    };
    useEffect(() => {
      load();
    ...
    const actionLabel = () =>
      !status
        ? "Loading..."
        : status.next_action === "clock_in"
          ? "Clock In"
          : status.next_action === "end_break"
            ? "End Break"
            : status.next_action === "clock_out"
              ? "Clock Out"
              : status.next_action === "completed"
                ? "Day Completed"
                : status.next_action === "pending_remote"
    ...
    const collectVerification = () =>
      new Promise((resolve) => {
        ...
        try {
          deviceId =
            localStorage.getItem("workonity_device_id") ||
    ...
    const uploadSelfie = () => {
      if (!me.settings?.verification_modules?.selfie_clockin || !selfie)
    ...
    const doAction = (action) => {
      ...
      Promise.all([
        collectVerification(),
        action === "clock_in" ? uploadSelfie() : Promise.resolve(""),
      ])
        .then(([verification, selfieRef]) => {
        ...
        .catch((err) => notifyError(err, "Attendance action failed"))
    ...
    const decideCorrection = (row, decision) => {
      ...
      focusEditPanel(
        ...
        decision === "approve"
          ? "Correction loaded for approval."
    ...
    const submitCorrectionDecision = () => {
      ...
      if (
        correctionDecision.decision === "reject" &&
        !String(correctionDecision.comments || "").trim()
      )
    ...
    const elapsed = (start) => {
      ...
      const seconds = Math.max(
        0,
        Math.floor(
          (nowTick - new Date(String(start).replace(" ", "T")).getTime()) /
            1000,
        ),
      );
      return [
    ...
    const workingElapsed = () => {
      ...
      let seconds =
        ...
        Number(status?.attendance?.total_break_minutes || 0) * 60;
      if (status?.open_break)
        seconds -= Math.max(
          0,
          Math.floor(
            (nowTick -
              new Date(
                String(status.open_break.break_in).replace(" ", "T"),
              ).getTime()) /
              1000,
          ),
        );
    ...
    const startSelfieCamera = async () => {
      ...
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        ...
        setSelfieCameraError(
          "This browser does not support camera capture. Choose an image only as a fallback.",
        );
        return;
      }
      ...
      try {
        ...
        if (error && error.name === "NotAllowedError") {
          ...
          setSelfieCameraError(
            "The live camera could not be opened. You may use an image only as a fallback.",
          );
        }
      }
    };
    const setSelfieFile = (file) => {
      ...
      if (!String(file.type || "").startsWith("image/"))
        return notifyError("Please choose an image file.");
      if (selfiePreview) URL.revokeObjectURL(selfiePreview);
      setSelfie(file);
    ...
    const captureSelfie = () => {
      ...
      canvas.toBlob(
        (blob) => {
          ...
          setSelfieFile(
            new File([blob], "workonity-clock-in-selfie.jpg", {
              type: "image/jpeg",
            }),
          );
        },
    ...
    useEffect(() => {
      if (verification.selfie_clockin && next === "clock_in" && !selfie)
        startSelfieCamera();
    ...
    return h(
      "div",
      null,
      hasAny(perms, ["attendance.clock", "attendance.manage"])
        ? h(
            "section",
            {
            ...
            h(
              ...
              h(
                ...
                status?.attendance?.clock_in && !status?.attendance?.clock_out
                  ? h(
                      "div",
                      {
                        className: "workonity-live-timers",
                      },
                      h("strong", null, "Working " + workingElapsed()),
                      status.open_break
                        ? h(
                            "strong",
                            null,
                            "Break " + elapsed(status.open_break.break_in),
                          )
                        : null,
                    )
                  : null,
              ...
              h(
                ...
                verification.selfie_clockin && next === "clock_in"
                  ? h(
                      "section",
                      {
                      ...
                      h(
                        ...
                        h(
                          "div",
                          null,
                          h("strong", null, "Live Clock-in Selfie"),
                          h(
                            "p",
                            null,
                            selfie
                        ...
                        selfie
                          ? h(
                              ...
                              {
                                className:
                                  "workonity-status workonity-status-approved",
                              },
                              "Captured",
                            )
                          : null,
                      ...
                      selfie
                        ...
                        : h(
                            ...
                            selfieCameraState === "requesting" ||
                              selfieCameraState === "idle"
                              ? h(
                                  "div",
                                  {
                      ...
                      selfieCameraError
                        ? h(
                            ...
                            h("p", null, selfieCameraError),
                            h(
                              "label",
                              {
                                className: "workonity-field",
                              },
                              h("span", null, "Fallback image"),
                              h("input", {
                                type: "file",
                  ...
                  : null,
                verification.qr_attendance && next === "clock_in"
                  ? h(Field, {
                  ...
                  : null,
                verification.remote_approval && next === "clock_in"
                  ? h(Checkbox, {
                ...
                h(
                  ...
                  next === "start_break_or_clock_out"
                    ...
                    : h(
                        ...
                        {
                          onClick: () =>
                            !["completed", "pending_remote"].includes(next) &&
                            doAction(next),
                          disabled:
                            loading ||
                            ["completed", "pending_remote"].includes(next),
        ...
        : null,
      hasAny(perms, ["attendance.correct", "attendance.manage"])
  ? h(
      "section",
      {
        className:
          "workonity-panel" +
          (correctionLocked ? " workonity-pro-locked-feature" : ""),
      },
      ...
      h(
        Button,
        {
          onClick: correctionLocked ? undefined : submitCorrection,
      ...
      correctionLocked
        ? h(
            "div",
            {
              className: "workonity-pro-lock-overlay",
            },

            h(
              "span",
              {
                className: "workonity-pro-lock-badge",
              },
              "PRO VERSION"
            ),

            h("strong", null, "Attendance Correction"),

            h(
              "p",
              null,
              "Upgrade to Workonity Pro to use this feature."
            )
          )
        : null
    )
  : null,
      hasAny(perms, ["attendance.manage", "attendance.manual"])
        ? h(
            "section",
            {
              className: "workonity-panel",
        ...
        h(DataTable, {
          ...
          renderRow: (r) =>
            h(
              ...
              h("td", null, r.attendance_date),
              h("td", null, r.employee_name || "-"),
              h("td", null, fmt(r.clock_in)),
              h("td", null, fmt(r.clock_out)),
              h("td", null, r.total_work_minutes || 0),
              h("td", null, r.total_break_minutes || 0),
              h(
                "td",
                null,
                h(Status, {
        ...
        ? h(
            ...
            correctionDecision.decision === "approve"
              ? h(
                  ...
                  h("h3", null, "Adjust Final Attendance Before Approval"),
                  h(
                    "p",
                    null,
                  ...
                  h(
                    ...
                    {
                      className:
                        "workonity-form-grid workonity-form-grid-compact",
                    },
                    h(Field, {
        ...
        h(DataTable, {
          ...
          renderRow: (c) => [
            h(
              ...
              h("td", null, c.employee_name || "Me"),
              h("td", null, c.request_type),
              h("td", null, fmt(c.requested_clock_in)),
              h("td", null, fmt(c.requested_clock_out)),
              h(
                "td",
                null,
                h(Status, {
                        ...
                        "button",
                        {
                          onClick: () =>
                            setExpandedCorrection(
                              expandedCorrection === c.id ? null : c.id,
                            ),
                        },
                        expandedCorrection === c.id
                          ? "Hide Details"
                          : "Details",
                      )
                    : null,
            ...
            expandedCorrection === c.id && c.correction_detail
              ? h(
                  "tr",
                  {
  ...
  function Leaves({ me }) {
    ...
    const load = () => {
      apiFetch({
        ...
        .then(setListState(setTypes))
        .catch(console.error);
      apiFetch({
        path: path("/leaves"),
      })
        .then(setListState(setLeaves))
        .catch(console.error);
    };
    useEffect(() => {
      load();
    ...
    const decide = (id, decision) => {
      ...
      if (decision === "reject" && !String(comments || "").trim())
        return notifyError("A rejection reason is required.");
      apiFetch({
      ...
      h(
        ...
        h(DataTable, {
          ...
          renderRow: (l) => {
            const canAct =
              l.status === "pending" &&
              ((String(l.active_approver_wp_user_id || "") &&
                String(l.active_approver_wp_user_id) ===
                  String(me.user?.id || me.user?.ID || "")) ||
                can(perms, "approvals.override"));
            const canCancel = can(perms, "leaves.manage")
              ? l.status === "pending" || l.status === "approved"
              : l.status === "pending" &&
                String(l.employee_id) === String(me.employee?.id || "");
            return h(
              "tr",
              {
                key: l.id,
              },
              h("td", null, l.employee_name || "Me"),
              h("td", null, l.leave_type_name),
              h("td", null, l.start_date),
              h("td", null, l.end_date),
              h("td", null, fmt(l.hours)),
              h(
                "td",
                null,
                h(Status, {
  ...
  function LeavesV2({ me }) {
    const perms = me.permissions || [];
    const canManageLeaves = can(perms, "leaves.manage");
    const canApproveLeaves = hasAny(perms, [
      "leaves.approve",
      "approvals.override",
    ]);
    const blankLeave = {
        ...
        h(
          ...
          canManageLeaves
            ? h(Field, {
                ...
                required: true,
                help: form.id
                  ? "Employee cannot be changed while editing an existing leave request."
                  : "",
              })
            : null,
          h(Field, {
          ...
          h(
            ...
            h("span", null, "Supporting Document"),
            h("input", {
              type: "file",
  ...
  function Employees({ me }) {
    ...
    const canManageOrganization = hasAny(me.permissions || [], [
      "departments.manage",
      "organization.manage",
    ]);
    const canManageShifts = hasAny(me.permissions || [], [
      "shifts.manage",
      "organization.manage",
    ]);
    const canManageUserPerms =
      hasProFeature("custom_roles") &&
      hasAny(me.permissions || [], ["roles.manage", "settings.manage"]);
    const canManageManagers = hasProFeature("organization_chart");
    const canManagePayrollFields =
      hasProFeature("payroll") &&
      can(me.permissions || [], "employees.sensitive");
    ...
    const load = () => {
      apiFetch({
        ...
        .then((data) => setEmployees(asArray(data)))
        .catch(console.error);
      apiFetch({
        path: path("/wp-users"),
      })
        .then((data) => setWpUsers(asArray(data)))
        .catch(console.error);
      apiFetch({
        path: path("/departments"),
      })
        .then((data) => setDepartments(asArray(data)))
        .catch(console.error);
      apiFetch({
        path: path("/designations"),
      })
        .then((data) => setDesignations(asArray(data)))
        .catch(console.error);
      apiFetch({
        path: path("/roles"),
      })
        .then((data) => setRoles(asArray(data)))
        .catch(console.error);
      apiFetch({
        path: path("/shifts"),
      })
        .then((data) => setShifts(asArray(data)))
        .catch(console.error);
      if (canManageUserPerms)
        apiFetch({
          path: path("/permissions"),
        })
          .then(setListState(setPermissions))
          .catch(console.error);
    };
    useEffect(() => {
      load();
    ...
    const submit = () =>
      apiFetch({
        ...
        .then((res) => {
          ...
          if (form.id && employeeId && canManageUserPerms)
            return apiFetch({
        ...
        .catch((e) => notifyError(e));
    const toggleEmployeePerm = (key, checked) =>
      setForm((current) => ({
        ...current,
        permission_overrides: checked
          ? Array.from(
              new Set((current.permission_overrides || []).concat(key)),
            )
    ...
    const addDepartmentOption = (created) =>
    ...
    const addDesignationOption = (created) =>
    ...
    return h(
      "div",
      null,
      canCreateEmployee || (form.id && canManageEmployees)
        ? h(
            "section",
            {
              className: "workonity-panel",
            ...
            h(PanelTitle, {
              ...
              text:
                canManagePayrollFields || canManageManagers
            ...
            h(
              ...
              canManagePayrollFields
                ? [
                    ...
                    form.pay_basis === "salary_commission"
                      ? h(Field, {
            ...
            !form.id && form.wp_user_id
              ? h(
                  ...
                  h("strong", null, "Existing WordPress account selected"),
                  h(
                    "p",
                    null,
                    "This employee will be linked to the selected account. A duplicate login will not be created.",
                  ),
                )
              : null,
            canManageManagers
              ? h(
                  ...
                  h(
                    ...
                    h(Field, {
                      ...
                      options: employees
                        .filter((e) => String(e.id) !== String(form.id))
                        .map((e) => ({
                          value: e.id,
              ...
              : null,
            form.id && canManageUserPerms
              ? h(
                  "div",
                  {
                    className: "workonity-subpanel",
                  },
                  h("h3", null, "User-specific Permission Exceptions"),
                  h(
                    "p",
                    null,
                  ...
                  h(
                    "div",
                    {
                      ...
                      style: {
                        opacity: form.permission_override_enabled ? 1 : 0.62,
                      },
                    },
                    Object.keys(permissionGroups).map((group) =>
                      h(
                        ...
                        permissionGroups[group].map((p) =>
                          h(Checkbox, {
                            key: p.permission_key,
                            label: p.label,
                            checked:
                              (form.permission_overrides || []).indexOf(
                                p.permission_key,
                              ) !== -1,
                            disabled: !form.permission_override_enabled,
                            onChange: (checked) =>
                              toggleEmployeePerm(p.permission_key, checked),
        ...
        h(DataTable, {
          ...
          renderRow: (e) =>
            h(
              ...
              h("td", null, e.employee_code || e.id),
              h(
                "td",
                null,
                h(
                  ...
                  h("span", null, fullName(e)),
                ),
              ),
              h("td", null, e.email),
              h("td", null, e.role_name || "-"),
              h("td", null, e.department_name || "-"),
              h("td", null, e.designation_name || "-"),
              h(
                "td",
                null,
                h(Status, {
  ...
  function EmployeesV2({ me }) {
    ...
    const upload = () => {
      if (!employeeId || !file)
        return notifyError("Choose an employee and image.");
      const fd = new FormData();
      fd.append("file", file);
      fetch(WORKONITY.root + `/employees/${employeeId}/photo`, {
        ...
        .then(({ response, data }) => {
          if (!response.ok || data.code)
            throw new Error(
              data.message || "The profile image could not be uploaded.",
            );
    ...
    return h(
      ...
      can(me.permissions || [], "employees.manage")
        ? h(
            "section",
            {
              className: "workonity-panel",
            ...
            h(
              ...
              h(
                ...
                h("span", null, "Profile Image"),
                selectedEmployee && selectedEmployee.profile_image_url
                  ? h("img", {
                      className: "workonity-photo-input-preview",
                ...
                h(
                  ...
                  file
                    ...
                    : "Choose a new image to upload or replace the current photo.",
                ),
              ),
            ),
            h(
  ...
  function NamedMasterPanel({ title, text, endpoint, columns }) {
    ...
    return h(
      ...
      h(
        ...
        form.id ? "Update " + title : "Add " + title,
      ),
      h(DataTable, {
  ...
  function ShiftsPanel() {
    ...
    const edit = (r) => {
      setForm({
        ...
        weekend_days: Array.isArray(r.weekend_days)
          ? r.weekend_days
          : (() => {
              try {
    ...
    const setShiftField = (key, value) =>
      setForm((current) => {
        ...
        if (calculated.valid && next.shift_type !== "flexible")
          next.working_minutes = calculated.working;
        return next;
      });
    ...
    const summary = timeline.valid
      ? h(
          ...
          h(
            ...
            h(
              "strong",
              null,
              timeline.endDay ? "Overnight shift" : "Same-day shift",
            ),
            h(
              "span",
              {
              ...
              timeline.endDay ? "Ends next day" : "Ends same day",
            ),
          ),
          h(
            "div",
            ...
            h(
              "div",
              null,
              h("small", null, "Shift starts"),
              h("strong", null, shiftTimeLabel(form.start_time)),
              h("span", null, "Shift date"),
            ),
            h(
              "div",
              null,
              h("small", null, "Shift ends"),
              h("strong", null, shiftTimeLabel(form.end_time)),
              h("span", null, timeline.endDay ? "Next day (+1)" : "Shift date"),
            ),
            h(
              "div",
              null,
              h("small", null, "Auto clock-out"),
              h("strong", null, shiftTimeLabel(form.auto_clockout_time)),
              h(
                "span",
                null,
                timeline.autoDay ? "Next day (+1)" : "Shift date",
              ),
            ),
            h(
              "div",
              null,
              h("small", null, "Calculated duration"),
              h("strong", null, shiftDurationLabel(timeline.working)),
              h(
                "span",
                null,
                `${shiftDurationLabel(timeline.gross)} gross - ${shiftDurationLabel(form.break_minutes)} break`,
              ),
            ),
          ),
        )
      : h(
          ...
          {
            className:
              "workonity-shift-timeline workonity-shift-timeline-error",
            role: "alert",
          },
          h("strong", null, "Timing needs attention"),
          h("span", null, timeline.error),
        );
    return h(
      "section",
      {
      ...
      h(
        ...
        h(Field, {
          ...
          help:
            timeline.valid && timeline.endDay
              ? "Automatically treated as next day (+1)."
        ...
        h(Field, {
          ...
          help:
            timeline.valid && timeline.lateDay
              ? "Late threshold is on the next day (+1)."
        ...
        h(Field, {
          ...
          help:
            form.shift_type === "flexible"
              ? "Set the expected minutes for this flexible shift."
        ...
        h(Field, {
          ...
          help: timeline.valid
            ? timeline.autoDay
              ? "This runs on the next day (+1)."
              : "This runs on the shift date."
            : "Must be at or after the scheduled end.",
        }),
        h(Field, {
      ...
      h(
        ...
        h("strong", null, "Weekend / Off Days"),
        h(
          "p",
          null,
          ...
          return h(
            ...
            h(
              ...
              h(
                "strong",
                null,
                `${shiftTimeLabel(r.start_time)} - ${shiftTimeLabel(r.end_time)}`,
              ),
              t.valid && t.endDay
                ? h(
                    "small",
            ...
            h(
              ...
              null,
              t.valid
                ? shiftDurationLabel(t.working)
                : `${r.working_minutes || 0} min`,
            ),
            h(
              "td",
              null,
              r.late_after_time
                ? shiftTimeLabel(r.late_after_time)
                : "Grace fallback",
            ),
            h(
              "td",
              null,
              shiftTimeLabel(r.auto_clockout_time),
              t.valid && t.autoDay
                ? h(
                    "small",
                    {
  ...
  function LeaveTypesPanel() {
    ...
    return h(
      ...
      h(
        ...
        form.id ? "Update Leave Type" : "Add Leave Type",
      ),
      h(DataTable, {
        columns: [
          "Name",
        ...
        renderRow: (r) =>
          h(
            ...
            h("td", null, r.name),
            h("td", null, r.annual_quota),
            h("td", null, r.carry_forward ? "Yes" : "No"),
            h("td", null, r.paid ? "Yes" : "No"),
            h("td", null, r.requires_attachment ? "Yes" : "No"),
            h(
              "td",
              null,
              h(Status, {
      ...
      h(DataTable, {
        ...
        renderRow: (r) =>
          h(
            ...
            h("td", null, r.name),
            h("td", null, r.first_year_quota),
            h("td", null, r.after_year_quota),
            h("td", null, r.balance_enforced ? "Enforced" : "Approval only"),
            h("td", null, r.carry_forward ? "Yes" : "No"),
            h("td", null, r.carry_forward_limit || "Unlimited"),
            h("td", null, r.paid ? "Yes" : "No"),
            h("td", null, r.requires_attachment ? "Yes" : "No"),
            h(
              "td",
  ...
  function HolidaysPanel() {
    ...
    const load = () => {
      ...
      apiFetch({
        ...
        .then(setListState(setDepartments))
        .catch(console.error);
    };
    useEffect(() => {
      load();
      ...
      h(DataTable, {
        ...
        renderRow: (r) =>
          h(
            ...
            h("td", null, r.holiday_date),
            h("td", null, r.title),
            h("td", null, r.type),
            h("td", null, r.department_name || "All Departments"),
            h(
              "td",
              null,
              h(
                  ...
                  grouped[dept].map((e) =>
                    h(
                      ...
                      h(
                        ...
                        null,
                        "Reports to: " +
                          (e.reporting_manager || "CEO by default"),
                      ),
                    ),
                  ),
                ),
  ...
  function OrgChartV2() {
    ...
    const structure = useMemo(() => {
      ...
      const ceo = rows.find(
        (e) => String(e.role_name || "").toLowerCase() === "ceo",
      );
      const roots = [];
      rows.forEach((e) => {
        let parent =
          e.managers && e.managers[0]
            ? String(e.managers[0].manager_employee_id)
            : "";
        if (!parent && ceo && String(e.id) !== String(ceo.id))
          parent = String(ceo.id);
        if (parent && byId[parent] && parent !== String(e.id))
          children[parent].push(e);
        else roots.push(e);
      });
      Object.keys(children).forEach((key) =>
        children[key].sort((a, b) => fullName(a).localeCompare(fullName(b))),
      );
      return {
        children,
        roots,
    ...
    const reportingLabel = (e) => {
      const primary =
        e.managers && e.managers[0] ? e.managers[0].manager_name : "";
      if (primary) return "Reports to " + primary;
      if (structure.ceo && String(e.id) !== String(structure.ceo.id))
        return "Reports to " + fullName(structure.ceo) + " (fallback)";
    ...
    const stats = useMemo(() => {
      ...
      rows.forEach((e) => {
        ...
        if (
          !(e.managers && e.managers[0]) &&
          !(structure.ceo && String(e.id) !== String(structure.ceo.id))
        )
          withoutManager++;
    ...
    const EmployeeCard = ({ employee, compact }) => {
      ...
      return h(
        "article",
        {
          className:
            "workonity-org-card" +
            (compact ? " workonity-org-card-compact" : "") +
        ...
        h(
          ...
          h(
            "span",
            null,
            employee.designation_name || employee.role_name || "-",
          ),
    ...
    const detailItem = (label, value) =>
      h("div", null, h("span", null, label), h("strong", null, fmt(value)));
      ...
      ? h(
          ...
          h(
            ...
            h(
              "div",
              null,
              h("h3", null, fullName(selected)),
              h(
                "p",
                null,
                selected.designation_name || selected.role_name || "-",
              ),
              h(Status, {
          ...
          h(
            ...
            detailItem(
              "Department",
              selected.department_name || "No department",
            ),
            detailItem("Designation", selected.designation_name),
            detailItem("Role", selected.role_name),
            detailItem(
              "Employment Type",
              String(selected.employment_type || "-").replace(/_/g, " "),
            ),
            detailItem("Joining Date", selected.joining_date),
            detailItem(
              "Reporting Manager",
              reportingLabel(selected).replace(/^Reports to /, ""),
            ),
            detailItem(
              "Direct Reports",
              (structure.children[String(selected.id)] || []).length,
            ),
          ),
          (structure.children[String(selected.id)] || []).length
            ? h(
                ...
                h("span", null, "Direct Team"),
                h(
                  "ul",
                  null,
                  (structure.children[String(selected.id)] || [])
                    .slice(0, 8)
                    .map((e) =>
                      h(
                        "li",
                        ...
                        h(
                          "small",
                          null,
                          e.designation_name || e.role_name || "-",
                        ),
                      ),
                    ),
                ),
            ...
            : null,
          (selected.managers || []).length > 1
            ? h(
                "div",
                {
                  className: "workonity-org-side-note",
                },
                h("span", null, "Additional reporting lines"),
                h(
                  "p",
                  null,
                  selected.managers
      ...
      : h(
          ...
          stats.biggest
            ? h(
                ...
                h("span", null, "Largest Team"),
                h(
                  "p",
                  null,
                  fullName(stats.biggest.employee) +
                    " - " +
                    stats.biggest.count +
                    " direct reports",
                ),
    ...
    return h(
      ...
      h(
        ...
        h(
          "label",
          null,
          h("span", null, "Search employees"),
          h("input", {
            type: "search",
            value: query,
            placeholder:
              "Search name, email, phone, designation, department...",
        ...
        rows.length
          ? h(
              ...
              sideOpen ? "Focus Tree" : "Show Details",
            )
          : null,
      ),
      rows.length
        ? h(
            ...
            {
              className:
                "workonity-org-split" + (sideOpen ? "" : " is-tree-focus"),
            },
            h(
              "div",
              {
              ...
              query.trim()
                ...
                : h(
                    ...
                    h(
                      "ul",
                      null,
                      structure.roots.map((e) => renderNode(e, new Set(), 0)),
                    ),
                  ),
            ),
            sideOpen ? detailPanel : null,
          )
        : h(EmptyState, {
  ...
  function Permissions() {
    ...
    useEffect(() => {
      ...
      apiFetch({
        ...
        .then(setListState(setSelectedPerms))
        .catch(console.error);
    ...
    const toggle = (key, checked) =>
      setSelectedPerms((old) =>
        checked
          ? Array.from(new Set(old.concat(key)))
          : old.filter((x) => x !== key),
      );
    const savePermsFor = (roleId) =>
      apiFetch({
        path: path(`/roles/${roleId}/permissions`),
        method: "POST",
        data: {
      ...
      h(DataTable, {
        ...
        renderRow: (r) =>
          h(
            ...
            h("td", null, `${r.object_type} #${r.object_id}`),
            h("td", null, r.employee_name || "-"),
            h(
              "td",
              null,
              r.approver_employee_id || r.approver_wp_user_id || "-",
            ),
            h("td", null, r.step_order),
            h(
              "td",
              null,
              h(Status, {
  ...
  function CorrectionDetails({ detail }) {
    if (!detail)
      return h(
        "div",
        {
    ...
    return h(
      ...
      h(
        ...
        h("h4", null, "Current Attendance"),
        actual.exists
          ? [
              cell("Date", actual.attendance_date),
              cell("Clock In", actual.clock_in),
              cell("Clock Out", actual.clock_out),
              cell("Status", actual.status, "status"),
              cell(
                "Work / Break",
                `${actual.total_work_minutes || 0} / ${actual.total_break_minutes || 0} min`,
              ),
              cell(
                "Late / Short",
                `${actual.late_minutes || 0} / ${actual.short_minutes || 0} min`,
              ),
            ]
          : h(
              "p",
      ...
      h(
        ...
        h("h4", null, "Employee Requested"),
        cell("Requested Clock In", requested.clock_in),
        cell("Requested Clock Out", requested.clock_out),
        h(
          "div",
          {
      ...
      h(
        ...
        h("h4", null, "Final Values for Approval"),
        cell("Final Clock In", approved.clock_in || requested.clock_in),
        cell("Final Clock Out", approved.clock_out || requested.clock_out),
        cell("Final Status", approved.status || "Auto calculate", "status"),
        h(
          "div",
          {
            className: "workonity-correction-note",
          },
          h("small", null, "Reviewer Note"),
          h("p", null, approved.reviewer_note || "-"),
        ),
      ),
    );
  }

  function LeaveApprovalDetails({ detail }) {
    if (!detail)
      return h(
        "div",
        {
    ...
    const openAttachment = (download) => {
      ...
      window.open(
        attachment.file_url +
          "?_wpnonce=" +
          encodeURIComponent(WORKONITY.nonce) +
          (download ? "&download=1" : ""),
        "_blank",
      );
    };
    return h(
      ...
      {
        className:
          "workonity-correction-details workonity-leave-approval-details",
      },
      h(
        "div",
        {
      ...
      h(
        ...
        {
          className:
            "workonity-correction-column workonity-correction-column-wide",
        },
        ...
        attachment
              ...
              h(
                ...
                [
                  ...
                  attachment.file_size
                    ? Math.ceil(Number(attachment.file_size) / 1024) + " KB"
                    : "",
                ]
                  .filter(Boolean)
                  .join(" | "),
          ...
          : h(
              ...
              h(
                "strong",
                null,
                Number(detail.requires_attachment)
        ...
        detail.latest_note
          ? h(
              ...
              h("small", null, "Latest Reviewer Note"),
              h("p", null, detail.latest_note),
            )
          : null,
      ),
    );
  ...
  function ApprovalsV2() {
    ...
    const submitDecision = () => {
      ...
      if (
        ...
        !String(pendingDecision.comments || "").trim()
      )
        return notifyError("A rejection reason is required.");
      apiFetch({
    ...
    return h(
      ...
      pendingDecision
        ? h(
            ...
            pendingDecision.row.object_type === "attendance_correction"
              ? h(CorrectionDetails, {
                  detail: pendingDecision.row.correction_detail,
                })
              : null,
            pendingDecision.row.object_type === "leave"
          ...
          renderRow: (r) => [
            h(
              ...
              h(
                "td",
                null,
                `${r.object_type.replace(/_/g, " ")} #${r.object_id}`,
              ),
              h("td", null, r.employee_name || "-"),
              h("td", null, (r.step_type || "manager").replace(/_/g, " ")),
              h("td", null, r.step_order),
              h(
                "td",
                null,
              ...
              h(
                ...
                h(
                  ...
                  r.correction_detail || r.leave_detail
                    ? h(
                        "button",
                        {
                          onClick: () =>
            ...
            expanded === r.id && (r.correction_detail || r.leave_detail)
              ? h(
                  "tr",
                  {
                    key: "details-" + r.id,
  ...
  function ReportsV2({ me }) {
    ...
    const reportFormats = WORKONITY.reportExportFormats ||
      me?.settings?.report_export_formats || {
        csv: true,
        excel: true,
        pdf: true,
      };
    const formatEnabled = (key) =>
      ![false, 0, "0"].includes(reportFormats[key]);
    const reportPermissions = me?.permissions || [];
    const canCsv =
      formatEnabled("csv") && can(reportPermissions, "reports.export");
    const canExcel =
      formatEnabled("excel") && can(reportPermissions, "reports.excel");
    const canPdf =
      formatEnabled("pdf") && can(reportPermissions, "reports.pdf");
    useEffect(() => {
      apiFetch({
        path: path("/departments"),
      })
    ...
    const endpoint = () =>
      attendanceKinds.includes(type)
        ? `/reports/attendance?kind=${type === "attendance" ? "" : type}`
        : type === "leaves"
          ? "/reports/leaves"
          : type === "payroll"
            ? "/reports/payroll"
    ...
    const summaryMetrics = useMemo(
      () => reportSummaryMetrics(type, summary),
      [type, summary],
    );
    ...
    const renderValue = (row, column) =>
      column.key === "status"
        ...
        : column.key === "leave_paid"
          ? Number(row[column.key])
            ? "Paid"
            : "Unpaid"
          : column.key === "salary_day_basis"
            ? {
              ...
              }[row[column.key]] || fmt(row[column.key])
            : fmt(row[column.key]);
    return h(
      "section",
      {
  ...
  function Payroll({ me }) {
    ...
    const [currency, setCurrency] = useState(
      me.settings?.payroll_policy?.payroll_output_currency ||
        me.settings?.default_currency ||
        "PKR",
    );
    ...
    const configuredDayBasis =
      me.settings?.payroll_policy?.salary_day_basis === "calendar_month"
        ? "calendar_month"
        : "custom_working_days";
    const configuredCustomDays = Number(
      me.settings?.payroll_policy?.salary_day_custom_days ||
        (me.settings?.payroll_policy?.salary_day_basis === "fixed_30"
          ? 30
          : me.settings?.payroll_policy?.salary_day_basis === "fixed_22"
            ? 22
            : me.settings?.payroll_policy?.working_days_divisor || 22),
    );
    ...
    const run = () => {
      if (generationScope === "employee" && !employeeId)
        return notifyError("Select an employee before generating payroll.");
      if (generationScope === "department" && !departmentId)
        return notifyError("Select a department before generating payroll.");
      return apiFetch({
        ...
        .then((result) => {
          if (result && result.already_generated) notifyInfo(result.message);
            ...
            notifySuccess(
              result && result.message ? result.message : "Payroll generated.",
            );
          return load();
        })
        .catch((e) => notifyError(e));
    ...
    const pdf = (id) =>
      window.open(
        WORKONITY.root +
          `/payroll/payslips/${id}/pdf?_wpnonce=${WORKONITY.nonce}`,
        "_blank",
      );
    return h(
      "div",
      ...
      edit && canManage
        ? h(
            ...
            h(
              ...
              edit.pay_basis === "salary_commission"
                ? h(Field, {
                    label: "Sales Amount",
                    type: "number",
                    step: "0.01",
              ...
              [
                ...
                h(Field, {
                  ...
                  disabled:
                    k === "commission_amount" &&
                    edit.pay_basis === "salary_commission" &&
                    edit.commission_type === "percentage",
                  help:
                    k === "commission_amount" &&
                    edit.pay_basis === "salary_commission"
        ...
        h(DataTable, {
          ...
          renderRow: (d) =>
            h(
              ...
              h("td", null, d.employee_name || d.employee_id),
              h("td", null, d.document_type),
              h("td", null, d.title),
              h("td", null, d.file_name || "Metadata only"),
              h(
                "td",
                null,
                h(Status, {
    ...
    return h(
      ...
      h(
        ...
        h(PanelTitle, {
          title: "Employee Documents",
          text: manage
            ? "All protected employee documents."
            : "Your protected employee documents.",
        }),
        h(DataTable, {
          ...
          renderRow: (d) =>
            h(
              ...
              h("td", null, d.employee_name || "Me"),
              h("td", null, d.document_type),
              h("td", null, d.title),
              h("td", null, fmt(d.expires_at)),
              h(
                "td",
                null,
                h(Status, {
  ...
  function AnnouncementsV2({ me }) {
    ...
    const save = () => {
      if (!String(form.title || "").trim())
        return notifyError("Announcement title is required.");
      apiFetch({
    ...
    return h(
      ...
      h(
        ...
        h(PanelTitle, {
          ...
          text: manage
            ? "All announcements."
            : "Announcements for your audience.",
        }),
        h(DataTable, {
          columns: manage
            ? ["Title", "Audience", "Status", "Published", "Actions"]
            : ["Title", "Audience", "Status", "Published"],
          rows,
          dateFilters: [
            {
              key: "published_at",
      ...
      h(DataTable, {
        ...
        renderRow: (n) =>
          h(
            ...
            h("td", null, n.message),
            h("td", null, n.type),
            h("td", null, n.read_at ? "Yes" : "No"),
            h("td", null, n.created_at),
            h(
              "td",
              null,
              !n.read_at
      ...
      h(
        ...
        qr
          ? h(
              ...
              h("code", null, qr.token),
              h("p", null, "Expires in five minutes."),
            )
          : null,
      ),
      h(
        ...
        h(DataTable, {
          ...
          renderRow: (d) =>
            h(
              ...
              h("td", null, d.employee_name || d.employee_id),
              h("td", null, d.device_label || d.device_hash),
              h(
                "td",
                null,
                h(Status, {
  ...
  function Settings() {
    ...
    const salaryMode =
      payroll.salary_day_basis === "calendar_month"
        ? "calendar_month"
        : "custom_working_days";
    const customDays =
      payroll.salary_day_custom_days ||
      (payroll.salary_day_basis === "fixed_30"
        ? 30
        : payroll.salary_day_basis === "fixed_22"
          ? 22
          : payroll.working_days_divisor || 22);
    return h(
      "div",
      null,
      h(
      ...
      h(
        ...
        h(
          ...
          h(Field, {
            ...
            onChange: (v) =>
              setNested("attendance_policy", "default_auto_clockout", v),
          }),
          h(Field, {
      ...
      h(
        ...
        h(
          ...
          [
            "enabled",
            "company_level_currency",
            "manual_adjustments",
            "auto_unpaid_leave_deduction",
            "auto_overtime",
            "requires_approval",
          ].map((k) =>
            h(Checkbox, {
        ...
        h(
          ...
          salaryMode === "custom_working_days"
            ? h(Field, {
                label: "Custom Working Days",
                type: "number",
                min: "1",
                ...
                value: customDays,
                onChange: (v) =>
                  setNested("payroll_policy", "salary_day_custom_days", v),
              })
            : null,
          h(Field, {
            ...
            onChange: (v) =>
              setNested("payroll_policy", "standard_daily_hours", v),
          }),
          h(Field, {
            ...
            onChange: (v) =>
              setNested("payroll_policy", "overtime_multiplier", v),
          }),
          h(Field, {
            ...
            onChange: (v) =>
              setNested("payroll_policy", "late_deduction_per_minute", v),
          }),
        ),
      ),
      h(
  ...
  function BrandingLogoControl({ value, onChange }) {
    const chooseLogo = () => {
      if (!window.wp || !window.wp.media) {
        notifyError(
          "The WordPress Media Library is still loading. Please refresh and try again.",
        );
        return;
      }
    ...
    const save = () => {
      const next = {
        ...
        payroll_policy: {
          ...
          exchange_rate_updated_at: (settings.payroll_policy || {})
            .usd_to_pkr_rate
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          palette.map(([key, label, fallback]) =>
            h(Field, {
              key,
              label,
              type: "color",
      ...
      h(
        ...
        h(
          ...
          salaryDayBasis === "custom_working_days"
            ? h(Field, {
                label: "Custom Working Days",
                type: "number",
                min: "1",
          ...
          h(Field, {
            ...
            onChange: (v) =>
              setNested("payroll_policy", "hourly_hours_source", v),
          }),
          h(Field, {
            label: "Payroll Output Currency",
            value:
              payroll.payroll_output_currency ||
              settings.default_currency ||
              "PKR",
            options: CURRENCY_OPTIONS,
            onChange: (v) =>
              setNested("payroll_policy", "payroll_output_currency", v),
          }),
          h(Field, {
      ...
      h(
        ...
        h(
          ...
          h(
            ...
            null,
            h("h4", null, "Before"),
            h("pre", null, auditPayloadText(row.old_value)),
          ),
          h(
            "div",
            null,
            h("h4", null, "After"),
            h("pre", null, auditPayloadText(row.new_value)),
          ),
          h(
            "div",
            null,
            ...
            h(
              ...
              null,
              "Actor user ID: " +
                fmt(row.actor_user_id) +
                " | Employee ID: " +
                fmt(row.actor_employee_id) +
                " | Raw action: " +
                fmt(row.action),
            ),
          ),
  ...
  function Audit({ me }) {
    ...
    const purge = () => {
      if (!preview || !preview.eligible_records)
        return notifyInfo("There are no audit records eligible for deletion.");
      const confirmation = window.prompt(
        (preview.eligible_records || 0) +
          " of " +
      ...
      h(PanelTitle, {
        ...
        actions: isSuperAdmin
          ? h(
              ...
              "Review and Purge Eligible Logs",
            )
          : null,
      }),
      isSuperAdmin && preview
        ? h(
            ...
            h(
              ...
              null,
              (preview.eligible_records || 0) +
                " currently eligible for manual purge. Up to " +
                (preview.batch_limit || 0) +
                " can be removed per approved operation.",
            ),
          )
        : null,
      h(DataTable, {
  ...
  function ProPreview() {
    return h(
      ...
      h(PanelTitle, {
        title: WORKONITY.proActive
          ? "WORKONITY Professional is connected"
          : "Unlock WORKONITY Professional",
        text: WORKONITY.proActive
          ? "Your Professional subscription is active."
      ...
      h(
        ...
        h(
          ...
          h(
            "strong",
            { className: "workonity-plan-title" },
            null,
            "Professional",
          ),
          h(
            ...
            null,
            "Advanced ",
            h("strong", null, "WORKONITY "),
            "capabilities for one website: payroll, approvals, secure documents, attendance verification, exports, and more.",
          ),
        ),
        h(
          "article",
          null,
          h("strong", { className: "workonity-plan-title" }, null, "Agency"),
          h(
            "p",
            null,
  ...
  function ProLocked({ feature, title }) {
    return h(
      "section",
      {
  ...
  function App() {
    ...
    useEffect(() => {
      const handleProRequirement = (event) => {
        const feature = String(
          event && event.detail && event.detail.feature
      ...
      return () =>
        window.removeEventListener(
          "workonity:pro-required",
          handleProRequirement,
        );
    }, []);
    if (error)
      return h(
    ...
    const proView = (view, node, title) => {
      ...
      return feature && !hasProFeature(feature)
        ? h(ProLocked, {
            feature,
            title,
          })
    ...
    const currentView = requiredProFeature
      ? h(ProLocked, {
  ...
  if (root) {
    try {
      if (wp.element.createRoot)
        wp.element.createRoot(root).render(h(ErrorBoundary, null, h(App)));
      else wp.element.render(h(ErrorBoundary, null, h(App)), root);
    } catch (e) {
      root.innerHTML =
        '<div class="workonity-error">Dashboard could not start. Check browser console.</div>';
      console.error(e);
    }
  }
})();
